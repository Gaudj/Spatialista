(ns spatialista.utils.validation
...
(defn set-error!
  "Explicitly set an error in the spatialista.validation/*errors* atom for
...
(defn clear-errors!
  ...
  in the spatialista.validation/*errors* atom."
  []
  (reset! *errors* {}))
...
(defn wrap-spatialista-validation [handler]
  (fn [request]
    (binding [*errors* (atom {})]
      (handler request))))
